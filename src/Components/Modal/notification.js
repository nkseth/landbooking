...
const reservation=useSelector((state) => state.reservartions.reservationdetails)
...
React.useEffect(
  ...
  let eventSource2 = new EventSourcePolyfill(`${baseurl}api/v1/event/restricted/subscribe/reservation_failed`
      ...
      eventSource2.onmessage = e => {setdata(JSON.parse(e.data)); 
        seteventtype("failed")
        eventSource2.close();
      }
...
const processcheckout=token=>{
 ...
 setdata(null);seteventtype(null)
  ...
  return (
    <>
      ...
      <Modal
        size="lg"
        show={props.show}
        onHide={() => {setdata(null);seteventtype(null) ;props.close()}}
      ...
      <h6 style={{backgroundColor:'red',textAlign:'center',padding:'10px 15px',color:'white'
      ,borderRadius:'10px',marginBottom:'20px'}}>
     
     {eventtype==="payment"?"Please Proceed with the payment":eventtype==="failed"? "Your request could not be processed please try again":'Please Wait we are processing your request....'} 
     </h6>
     {eventtype==="payment" &&
     <>
     <div>
       ...
       {venue?.images[0]?
       <img alt="venue" src={baseurl+venue?.images[0]} 
     ...
     <h6 > Guest List </h6>
      <div className="d-flex justify-content-between">
       <p style={{fontSize: "12px"}}>Adults:{reservation?.guestList?.adults}</p>
       <p  style={{fontSize: "12px"}}>Children:{reservation?.guestList?.children}</p>
       <p  style={{fontSize: "12px"}}>Infants:{reservation?.guestList?.infants}</p>
       </div>
     </div>
...
{reservation?.schedules.map((item)=>{
  ...
  {item.intervals.map((item2)=>{
   return <p className="mx-2 "  style={{fontSize: "12px"}}>{item2}</p>
  })}
  </div>
</div>
})
...
{reservation?.slots?reservation.slots.map((item)=>{
  return <p className="mx-2"  style={{fontSize: "12px"}}>{item}</p>
})
:null}
</div>
       </div>
